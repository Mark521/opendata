...
class Window:
    ...
    def packBtn(self):
        ...
        self.match = Button(self.frm_RT, text = '匹配数据', command=lambda :self.matchData('d'), width=15, height=2 )
    ...
    def matchData(self, data):
        ...
        self.top_str_main = Label(self.top_frm_T, text='数据条目2', font=('Arial', 15)).pack(side=LEFT)

        self.top_frm_M = Frame(self.top)
        Label(self.top_frm_M, text='数据条目3',padx = 10, font=('Arial', 15)).pack(side=RIGHT)
        self.top_str_secondary = Label(self.top_frm_M, text='数据条目4', font=('Arial', 15)).pack(side=RIGHT)
        
        self.top_frm_B = Frame(self.top)
        Button(self.top_frm_B, text = '上一条',command=self.matchData, width=10, height=2).pack(side=LEFT)
        check = PhotoImage(file = 'check.png')
        error = PhotoImage(file = 'error.png')
        Button(self.top_frm_B, image = check,command=self.matchData, width=30, height=30).pack(side=LEFT)
        Button(self.top_frm_B, image = error,command=self.matchData, width=30, height=30).pack(side=LEFT)
        #Label(self.top_frm_B, text='数据条目5',bitmap=bm).pack(side=LEFT)
        Button(self.top_frm_B, text = '下一条',command=self.top.destroy, width=10, height=2).pack(side=LEFT)
