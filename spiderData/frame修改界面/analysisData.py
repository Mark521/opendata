...
def matchList(entrys1, entrys2):
    ...
    for e in entrys1:
        for a in entrys2:
            ...
            match = e.matchEditDis(a)
            
            e.setFlag(match)
            a.setFlag(match)
            #if match != 0 and match/size >=0.618:
            if match >=0.618:
                print(match, e.name, a.name)
                match1 = []
                match1.insert(0, match)
                match1.insert(1, e)
                match1.insert(2, a)
                matchL.append(match1)
            else:
                match2 = []
                match2.insert(0, match)
                match2.insert(1, e)
                match2.insert(2, a)
                noMatchL.append(match2)
            #match1.sort(key=lambda x:x.flag,reverse = True)
            #match2.sort(key=lambda x:x.flag,reverse = True)
    return matchL, noMatchL
...
if __name__ == '__main__':
    ...
    #进行数据匹配
    matchL, noMatchL = matchList(entryBJ, entrySH)
    matchL.sort(key=lambda x:x[0], reverse = True)
    noMatchL.sort(key=lambda x:x[0], reverse = True)
    #准备每个城市的分词列表
