...
def saveAnalyFile(fname, filename):
...
def checkMode():
  ...
  if w.model.get() == 1:
    ...
    saveAnalyFile(fname, filename)

class Window(Frame):
    
    def __init__(self, master=None):
        self.root = Tk(className='表格数据合并')
        
    def config(self):
        ws = self.root.winfo_screenwidth()
        hs = self.root.winfo_screenheight()
        self.root.geometry('250x100+{}+{}'.format(int(ws/2 - 125), int(hs/2 - 50)))
        self.model = IntVar()
        self.model.set(1)
        self.first = Radiobutton(self.root, text='模式一',variable = self.model, value = 1,anchor=W)
        self.first.place(relx=0.2)
        self.second = Radiobutton(self.root, text='模式二',variable = self.model, value = 2,anchor=W)
        self.second.place(relx=0.5)
        #self.entry = Entry(self.root, textvariable=self.model)
        #self.entry.place(rely=0.8)
        
        Button(self.root,command=checkMode,  text='分析文件', font=('Arial', 15), width=15, height=2,anchor='center').place(relx=0.15, rely=0.3)
        self.root.mainloop()
